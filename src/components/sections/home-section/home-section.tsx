...
export default function HomeSection(): JSX.Element {
  ...
  const generateBubbles = (): IBubble[] => {
    return new Array(100).fill({}).map((x, i) => {
      const radius = getRandomNumber(0, 50);
      const translatedCoordinates = {
        x: getRandomNumber(0, window.innerWidth),
        y: getRandomNumber(0, window.innerHeight - 100),
      };
      const fill = i % 2 === 0 ? fillColors[0] : fillColors[1];
  ...
  return (
    <section className='home-container' id='home-section'>
      <svg className='circle-layer'>
        {bubbles.map(({ fillColor, translatedCoordinates, radius }, i) => (
          <g
            key={i}
            transform={`translate(${translatedCoordinates.x},${translatedCoordinates.y})`}
          >
            <circle
              fill={fillColor}
              cx={radius}
            ...
            >
              <animate
                attributeName='cx'
                values={`0;${getRandomNumber(-45, 45)};0;}`}
                dur='30s'
